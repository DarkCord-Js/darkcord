...
(function discordio (Discord) {
  const isNode = typeof (window) === 'undefined' && typeof (navigator) === 'undefined'
  let CURRENT_VERSION = '2.x.x'
  const GATEWAY_VERSION = 6
  const LARGE_THRESHOLD = 250
  let CONNECT_WHEN = null
  let Endpoints; let Payloads
 ...
 */
  DCP.editUserInfo = function (input, callback) {
    ...
    const plArr = Object.keys(payload)
 ...
 */
  DCP.setPresence = function (input) {
    const payload = Payloads.STATUS(input)
 ...
 */
  DCP.uploadFile = function (input, callback) {
    ...
    let file, client, multi, message, isBuffer, isString
 ...
 */
  DCP.sendMessage = function (input, callback) {
    const message = generateMessage(input.message || '', input.embed)
 ...
 */
  DCP.getMessages = function (input, callback) {
    const client = this; const qs = {}; let messages = []; let lastMessageID = ''
    let total = typeof (input.limit) !== 'number' ? 50 : input.limit
 ...
 */
  DCP.getReaction = function (input, callback) {
    const qs = { limit: (typeof (input.limit) !== 'number' ? 100 : input.limit) }
    const client = this
    resolveID(client, input.channelID, function (channelID) {
 ...
 */
  DCP.ban = function (input, callback) {
    let url = Endpoints.BANS(input.serverID, input.userID)
    const opts = {}
 ...
 */
  DCP.muteSelf = function (serverID, callback) {
    const server = this.servers[serverID]; let channelID; let voiceSession
 ...
 */
  DCP.editServer = function (input, callback) {
    let payload; const serverID = input.serverID; let server; const client = this
 ...
 */
  DCP.editServerWidget = function (input, callback) {
    const client = this; let payload; const url = Endpoints.SERVERS(input.serverID) + '/embed'
 ...
 */
  DCP.createInvite = function (input, callback) {
    const allowed = ['channelID', 'max_age', 'max_uses', 'temporary', 'unique']
    const payload = {}
 ...
 */
  DCP.createChannel = function (input, callback) {
    const client = this; const payload = {
      name: input.name,
    ...
    this._req('post', Endpoints.SERVERS(input.serverID) + '/channels', payload, function (err, res) {
      try {
        const serverID = res.body.guild_id
 ...
 */
  DCP.editChannelPermissions = function (input, callback) { // Will shrink this up later
    let payload, pType, ID, channel, permissions, allowed_values
 ...
 */
  DCP.editNickname = function (input, callback) {
    ...
    const url = input.userID === this.id
 ...
 */
  DCP.editWebhook = function (input, callback) {
    const client = this; const payload = {}; const allowed = ['avatar', 'name']
 ...
 */
  DCP.joinVoiceChannel = function (channelID, callback) {
    let serverID, server, channel, voiceSession
    try {
 ...
 */
  DCP.getAudioContext = function (channelObj, callback) {
    ...
    const channelID = channelObj.channelID || channelObj; const voiceSession = this._vChannels[channelID]; const encoder = chooseAudioEncoder(['ffmpeg', 'avconv'])
 ...
 */
  DCP.getAllUsers = function (callback) {
    const servers = Object.keys(this.servers).filter(function (s) {
      s = this.servers[s]
  ...
  function handleResCB (errMessage, err, res, callback) {
    ...
    const e = new Error(err || errMessage)
  ...
  function cacheMessage (cache, limit, channelID, message) {
    ...
    const k = Object.keys(cache[channelID])
  ...
  function stringifyEmoji (emoji) {
    ...
    if (emoji.indexOf(':') > -1) { return emoji }
    return encodeURIComponent(decodeURIComponent(emoji))
  }

  /* - Functions - Utils */
  function APIRequest (method, url) {
    let data; let callback; let opts; let req; const headers = messageHeaders(this)
  ...
  function qstringify (obj) {
    ...
    let i = 0; let s = ''; const k = Object.keys(obj)
    for (i; i < k.length; i++) {
      s += k[i] + '=' + obj[k[i]] + '&'
    }
    return '?' + s.slice(0, -1)
  ...
  function emit (client, message) {
    if (!message.t) return
    const t = message.t.split('_'); let i = 1; const args = [t[0].toLowerCase()]
  ...
  function removeAllListeners (emitter, type) {
    if (!emitter) return
    const e = emitter._evts; let i; let k; let o; let s; let z
  ...
  function resolveID (client, ID, callback) {
    ...
    if (client.users[ID]) {
      return client.createDMChannel(ID, function (err, res) {
  ...
  function checkForAllServers (client, ready, message) {
    const all = Object.keys(client.servers).every(function (s) {
  ...
  function identifyOrResume (client) {
    let payload; const internals = client.internals
  ...
  function handleWSMessage (data, flags) {
    const message = decompressWSMessage(data, flags)
    const _data = message.d
    const client = this; let user; let server; let member; let old
    let userID; let serverID; let channelID; let currentVCID
    let mute; let deaf; let self_mute; let self_deaf
    ...
    switch (message.t) {
      ...
      case 'GUILD_MEMBERS_CHUNK':
        ...
        _data.members.forEach(function (member) {
          ...
          const members = client.servers[serverID].members
      ...
      case 'GUILD_SYNC':
        ...
        _data.presences.forEach(function (presence) {
          const uID = presence.user.id
          const members = client.servers[_data.id].members
  ...
  function handleWSClose (code, data) {
    const client = this
    const eMsg = Discord.Codes.WebSocket[code] || data
  ...
  function joinVoiceChannel (client, voiceSession) {
    let vWS; let vUDP; const endpoint = voiceSession.endpoint.split(':')[0]
    ...
    if (isNode) voiceSession.keepAliveBuffer = Buffer.from(8).fill(0)
    vWS = voiceSession.ws.connection = new Websocket('wss://' + endpoint)

    if (isNode) {
      return DNS.lookup(endpoint, function (err, address) {
  ...
  function handlevWSMessage (voiceSession, vMessage, vFlags) {
    const client = this; const vData = decompressWSMessage(vMessage, vFlags)
    switch (vData.op) {
  ...
  function handlevWSClose (voiceSession) {
    ...
    const audio = voiceSession.audio; const members = voiceSession.members
    if (audio) {
  ...
  function handleUDPMessage (voiceSession, msg, rinfo) {
    const buffArr = JSON.parse(JSON.stringify(msg)).data; const client = this; let vDiscIP = ''; let vDiscPort
  ...
  AudioCB.VoicePacket = (function () {
    if (!isNode) return
    const header = new Buffer(12); const nonce = new Uint8Array(24); const output = new Buffer(2048)
  ...
  ACBP.playAudioFile = function (location, callback) {
    ...
    const encs = ['ffmpeg', 'avconv']; let selection; let enc; const ACBI = this
  ...
  function prepareAudio (ACBI, readableStream, cnt) {
    const data = readableStream.read(320) || readableStream.read() // (128 [kb] * 20 [frame_size]) / 8 == 320
  ...
  function sendAudio (ACBI, buffer) {
    ...
    const audioPacket = AudioCB.VoicePacket(buffer, ACBI._voiceSession.ws.ssrc, ACBI._sequence, ACBI._timestamp, ACBI._secretKey)
  ...
  function handleIncomingAudio (msg) {
    ...
    const ssrc = header.readUIntBE(8, 4)
    const member = this._voiceSession.translator[ssrc]
    let decrypted; let decoded
  ...
  function Server (client, data) {
    ...
    data.voice_states.forEach(function (vs) {
      const cID = vs.channel_id
      const uID = vs.user_id
  ...
  function Channel (client, server, data) {
    ...
    data.permission_overwrites.forEach(function (p) {
      const type = (p.type === 'member' ? 'user' : 'role')
  ...
  Member.update = function (client, server, data) {
    ...
    const member = server.members[data.user.id]
  ...
  Object.defineProperty(User.prototype, 'avatarURL', {
    get: function () {
      ...
      const animated = this.avatar.indexOf('a_') > -1
  ...
  (function () {
    ...
    const CDN = 'http://cdn.discordapp.com'
